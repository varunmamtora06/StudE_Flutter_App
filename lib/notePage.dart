...
class _notePageState extends State<notePage> {
  ...
  void showToast() {
    showDialog(
        ...
        builder: (BuildContext context) {
          return AlertDialog(
            content: Text(
              '1. Swipe to delete a Note.\n2. Tap to (un)check.\n3. Long press to edit a Note.',
  ...
  Widget buildItem(Todo item, index) {
    return Dismissible(
      key: UniqueKey(),
      /*key:uniquekey bcuz there was dissmissable still part of tree error(solution found on stackowrflow)*/
  ...
  void saveData() {
    List<String> spList =
        items.map((item) => json.encode(item.toMap())).toList();
